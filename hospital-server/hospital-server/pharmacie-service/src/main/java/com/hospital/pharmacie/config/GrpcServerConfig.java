...
public class GrpcServerConfig {
    ...
    @Value("${grpc.pharmacie.port:50056}") private int port;
    @Value("${server.ip:192.168.1.10}") private String serverIp;
    private Server server;
    @Bean public CommandLineRunner start(PharmacieGrpcService svc) {
        return args -> {
            ...
            log.info("SERVICE PHARMACIE gRPC démarré sur {}:{}", serverIp, port);
