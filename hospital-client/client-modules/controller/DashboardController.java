...
public class DashboardController {
    ...
    private void setupMedecinUI() {
        ...
        setStat(1, "👥", "12", "Patients du jour", "Aujourd'hui");
        setStat(2, "📅", "5", "Rendez-vous", "Aujourd'hui");
        setStat(3, "🩺", "3", "Consultations en attente", "À traiter");
        setStat(4, "🔬", "2", "Résultats analyses", "Disponibles");
        setStat(5, "💊", "4", "Prescriptions en cours", "Actives");
        setStat(6, "📋", "7", "Dossiers ouverts", "Cette semaine");
        hideStatCards(false);
        ...
        welcomeHint.setText("Gérez vos consultations, dossiers médicaux, ordonnances et demandes d'analyses.");
    ...
    private void buildMedecinMain() {
        ...
        left.getChildren().add(buildStatusList(new String[][]{
            {"Jean Mukendi", "09:00", "Consultation générale", "En attente", "warning"},
            {"Marie Kabeya", "09:30", "Cardiologie", "En consultation", "info"},
            {"Pierre Ilunga", "10:15", "Suivi post-op", "Terminé", "success"}
        }, new String[]{"Patient", "Heure", "Motif", "Statut"}));

        VBox right = createCard("Patients récents");
        right.setPrefWidth(420);
        right.getChildren().add(buildSimpleList(new String[][]{
            {"Jean Mukendi", "45 ans · M · Hypertension"},
            {"Marie Kabeya", "32 ans · F · Contrôle grossesse"},
            {"Paul Ndaye", "58 ans · M · Diabète type 2"}
        }));
    ...
    public void refreshStats() {
        Task<Void> task = new Task<>() {
            @Override protected Void call() {
                try {
                    AccueilClient c = new AccueilClient();
                    int n = c.getPatients(false).size();
                    Platform.runLater(() -> {
                        if (currentRole == UserSession.Role.ADMIN || currentRole == UserSession.Role.MEDECIN
                                || currentRole == UserSession.Role.RECEPTIONNISTE)
                            statValue1.setText(String.valueOf(n));
                    });
                } catch (Exception ignored) {}
                try {
                    HospitalisationClient h = new HospitalisationClient();
                    int n = h.getHospitalisations(null, "EN_COURS").size();
                    Platform.runLater(() -> {
                        if (currentRole == UserSession.Role.ADMIN) statValue2.setText(String.valueOf(n));
                        else if (currentRole == UserSession.Role.RECEPTIONNISTE) statValue4.setText(String.valueOf(n));
                    });
                } catch (Exception ignored) {}
                try {
                    PharmacieClient p = new PharmacieClient();
                    int n = p.getMedicaments(false).size();
                    Platform.runLater(() -> {
                        if (currentRole == UserSession.Role.PHARMACIEN) statValue1.setText(String.valueOf(n));
                        else if (currentRole == UserSession.Role.ADMIN) statValue5.setText(String.valueOf(n));
                    });
                } catch (Exception ignored) {}
                return null;
            }
    ...
    private void showPlaceholder(String name) {
        ...
        t.setStyle("-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: #1e293b;");
        Label s = new Label("Module en cours de finalisation. Clients gRPC prêts.");
    ...
    @FXML private void showDossiers() { setActive(btnDossiers, "Dossiers médicaux"); showPlaceholder("Dossiers médicaux"); }
    @FXML private void showOrdonnances() { setActive(btnOrdonnances, "Ordonnances"); loadView("/fxml/pharmacie.fxml"); }
    @FXML private void showResultats() { setActive(btnResultats, "Résultats"); loadView("/fxml/laboratoire.fxml"); }
    @FXML private void showRendezVous() { setActive(btnRendezVous, "Rendez-vous"); showPlaceholder("Rendez-vous"); }
    @FXML private void showHistorique() { setActive(btnHistorique, "Historique"); showPlaceholder("Historique"); }
    ...
    @FXML private void showFileAttente() { setActive(btnFileAttente, "File d'attente"); showPlaceholder("File d'attente"); }
    ...
    @FXML private void showStocks() { setActive(btnStocks, "Stocks"); loadView("/fxml/pharmacie.fxml"); }
    ...
    @FXML private void showRecus() { setActive(btnRecus, "Reçus"); showPlaceholder("Reçus"); }
    @FXML private void showRapports() { setActive(btnRapports, "Rapports"); showPlaceholder("Rapports"); }
    @FXML private void showMaternite() { setActive(btnMaternite, "Maternité"); showPlaceholder("Maternité"); }
    @FXML private void showSettings() { setActive(btnSettings, "Paramètres"); loadView("/fxml/settings.fxml"); }
    @FXML private void showProfil() { setActive(btnProfil, "Profil"); showPlaceholder("Profil utilisateur"); }
